...
export function getWeaponMatchScore(weaponSkinName, knifeName, gloveName) {
  ...
  // Helper function to calculate match score using NEW algorithm
  const calculateMatchScore = (weaponData, targetData, targetName) => {
    ...
    // NEW WEIGHTS: Using SCORE_WEIGHTS for balanced scoring
  ...
  if (knifeName) {
    const knife = getSkinData(knifeName);
    knifeScore = calculateMatchScore(weapon, knife, knifeName);
  ...
  if (gloveName) {
    const glove = getSkinData(gloveName);
    gloveScore = calculateMatchScore(weapon, glove, gloveName);
