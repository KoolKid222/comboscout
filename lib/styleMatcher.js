...
import { generatedSkinColors } from './skinColors/generatedColors';
...
// Merge all color databases
// Order matters: generated colors are overridden by manual colors
const skinColors = {
  ...generatedSkinColors,  // Auto-extracted colors (983 skins)
  ...knifeGloveSkinColors, // Manual knife/glove colors (priority)
  ...weaponSkinColors,     // Manual weapon colors (priority)
};

// Pre-compute sorted keys and lowercase versions for fast lookups
// This avoids sorting and toLowerCase() on every getSkinData() call
const sortedSkinColorKeys = Object.keys(skinColors).sort((a, b) => b.length - a.length);
const sortedSkinColorKeysLower = sortedSkinColorKeys.map(k => k.toLowerCase());
...
/**
 * Calculate overall color harmony score between two HSL colors (CREATIVE MODE)
 * Used for knife/glove matching where creative contrast is valued
 * Returns 0-1 score
 *
 * PRIORITY ORDER (values artistic contrast):
 * 1. Analogous (< 40°): 0.85 - Very harmonious
 * 2. Complementary (~180°): 0.75 - High contrast is creative/bold
 * 3. Split-complementary: 0.72 - Dynamic tension
 * 4. Triadic (~120°): 0.70 - Vibrant combinations
 * 5. Clash zone (60-90°): 0.35 - Bad pairing
 */
function calculateColorHarmony(hsl1, hsl2) {
  ...
  // Both chromatic - apply CREATIVE color theory
  const hueDiff = hueDifference(h1, h2);

  // ANALOGOUS - Adjacent colors on wheel (< 40°)
  if (isAnalogous(h1, h2)) {
    harmonyScore = 0.85; // Very harmonious - similar colors
  }
  // COMPLEMENTARY - Opposite colors ~180° (CREATIVE - high contrast is GOOD)
  // Red + Green, Blue + Orange = bold artistic statement
  else if (isComplementary(h1, h2)) {
    harmonyScore = 0.75; // High contrast valued for knife/glove combos
  }
  // SPLIT-COMPLEMENTARY - ~150° apart
  else if (isSplitComplementary(h1, h2)) {
    harmonyScore = 0.72; // Dynamic tension, still creative
  }
  // TRIADIC - 120° apart
  else if (isTriadic(h1, h2)) {
    harmonyScore = 0.70; // Vibrant combinations
  }
  // Non-harmonic range - check if close or clashing
  else {
    if (hueDiff < 60 || hueDiff > 300) {
      harmonyScore = 0.55; // Close-ish hues
    } else {
      harmonyScore = 0.35; // Clash zone - neither coordinated nor complementary
    }
  }
  ...
  if (lightDiff > 20 && lightDiff < 50) {
    harmonyScore += 0.03; // Good contrast
...
/**
 * STRICT color harmony for weapon matching
 * Prioritizes monotone coordination over contrast
 * Returns 0-1 score
 *
 * PRIORITY ORDER (monotone beats contrast):
 * 1. Exact/Near-exact match (< 15°): 1.0 - Perfect coordination
 * 2. Analogous (< 40°): 0.85 - Very harmonious
 * 3. Close hues (< 60°): 0.70 - Good coordination
 * 4. Triadic (~120°): 0.55 - Vibrant but risky
 * 5. Complementary (~180°): 0.45 - HEAVILY NERFED - contrast = bad for weapons
 * 6. Split-complementary: 0.43 - Also nerfed
 * 7. Clash zone: 0.35 - Bad pairing
 */
function calculateWeaponColorHarmony(hsl1, hsl2) {
  const [h1, s1, l1] = hsl1;
  const [h2, s2, l2] = hsl2;
  ...
  if (s1 < 15 && s2 < 15) {
    const lightContrast = Math.abs(l1 - l2);
    return lightContrast > 30 ? 0.7 : 0.5;
  }
  ...
  // STRICT PRIORITY ORDER - monotone coordination over artistic contrast

  // EXACT/NEAR-EXACT MATCH - Best possible score
  // Red + Red, Blue + Blue = perfect coordination
  if (hueDiff <= 15) {
    return 1.0;
  }
  ...
  // CLOSE HUES - Not quite analogous but still coordinated (40-60°)
  if (hueDiff < 60) {
    return 0.70;
  }

  // TRIADIC - 120° apart (nerfed for weapons)
  if (hueDiff >= 100 && hueDiff <= 140) {
    return 0.55;
  }

  // COMPLEMENTARY - Opposite colors ~180° (HEAVILY NERFED for weapons)
  // Red vs Green = "Christmas clash" - BAD for coordinated loadouts
  if (hueDiff >= 150 && hueDiff <= 210) {
    return 0.45;
  }

  // SPLIT-COMPLEMENTARY - ~130-160° or ~200-230° apart (also nerfed)
  if ((hueDiff >= 130 && hueDiff < 150) || (hueDiff > 210 && hueDiff <= 230)) {
    return 0.43;
  }

  // CLASH ZONE - everything else
  return 0.35;
}
...
function getSkinData(itemName) {
  ...
  // Use pre-sorted and pre-lowercased keys for fast lookup
  for (let i = 0; i < sortedSkinColorKeys.length; i++) {
    if (name.includes(sortedSkinColorKeysLower[i])) {
      const key = sortedSkinColorKeys[i];
  ...
  if (parts.length > 1) {
    ...
    for (let i = 0; i < sortedSkinColorKeys.length; i++) {
      const keyLower = sortedSkinColorKeysLower[i];
      if (patternName.includes(keyLower) || keyLower.includes(patternName)) {
        const key = sortedSkinColorKeys[i];
        return { name: key, ...skinColors[key] };
      }
    }
...
export function getWeaponMatchScore(weaponSkinName, knifeName, gloveName) {
  ...
  const calculateMatchScore = (weaponData, targetData, targetName) => {
    // STEP 1: Calculate primary color harmony FIRST (using STRICT weapon harmony)
    const primaryHarmony = calculateWeaponColorHarmony(weaponData.hsl, targetData.hsl);
    ...
    // STEP 3: Primary passed the gate - now calculate bonuses (using STRICT weapon harmony)
    const secondaryHarmony = calculateWeaponColorHarmony(weaponData.secondary, targetData.secondary);
    ...
    // NOTE: Theme bonus removed - name matching was causing false positives
    // (e.g., "Tiger Tooth" knife matching "Tiger Toof" glock despite different colors)
    ...
    let score = SCORE_WEIGHTS.base + primaryPoints + secondaryPoints + crossPoints + texturePoints;
